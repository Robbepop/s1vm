...
impl State {
    ...
    fn pop(&mut self) -> Result<Input> {
        self.values
            .pop()
            .ok_or_else(|| Error::ValidationError(format!("Value stack empty")))
    }
...
impl Compiler {
    ...
    fn compile_function(&mut self, func_idx: u32) -> Result<()> {
        ...
        let func = self.module.get_func(func_idx).ok_or(Error::FuncNotFound)?;
        ...
        self.compiled.push(Function::new(
            func,
            Box::new(
                move |state: &vm::State, store: &mut Store| -> Trap<Option<StackValue>> {
    ...
    fn emit_return(&self, state: &mut State, block: &mut Block) -> Result<()> {
        if self.ret_type.is_some() {
            ...
            match ret {
                Input::Local(local_idx) => {
                    block.push(Box::new(
                        move |_state: &vm::State, store: &mut Store| -> Trap<Action> {
                ...
                Input::Const(const_val) => {
                    block.push(Box::new(
                        move |_state: &vm::State, _store: &mut Store| -> Trap<Action> {
                            let ret = const_val;
                            Ok(Action::Return(Some(StackValue(ret.0 as _))))
                        },
                    ));
                ...
                Input::Op(closure) => {
                    block.push(Box::new(
                        move |state: &vm::State, store: &mut Store| -> Trap<Action> {
    ...
    fn compile_if(&self, parent: &mut Block, state: &mut State) -> Result<()> {
        ...
        let else_block = match &self.code[state.pc] {
            Else => Some(self.compile_else(state)?),
            End => None,
            _ => {
...
macro_rules! impl_int_binops {
    ($name: ident, $type: ty, $op: ident) => {
        pub fn $name(state: &mut State) -> Result<()> {
            ...
            let left = state.pop()?;
            match left {
                Input::Local(left_idx) => match right {
                    Input::Const(right_const) => {
                        state.push(Input::Op(Box::new(
                            move |_state: &vm::State, store: &mut Store| -> Trap<StackValue> {
    ...
    ($name: ident, $type: ty, $op: ident, $as_type: ty) => {
        pub fn $name(store: &mut Store) -> Trap<()> {
            store.stack.binop(|left, right| {
                let res = (left.0 as $type).$op(right.0 as $type).ok_or_else(|| {
...
macro_rules! impl_numeric_ops {
    ($op_mod: ident, $type: ty, $type_u: ty) => {
        ...
        mod $op_mod {
            ...
            impl_int_relops!(eqz, $type, |val| { val == Default::default() });
            impl_int_relops!(eq, $type, $type, |left, right| { left == right });
            impl_int_relops!(ne, $type, $type, |left, right| { left != right });
            impl_int_relops!(lt_s, $type, $type, |left, right| { left < right });
            impl_int_relops!(lt_u, $type_u, $type_u, |left, right| { left < right });
            impl_int_relops!(gt_s, $type, $type, |left, right| { left > right });
            impl_int_relops!(gt_u, $type_u, $type_u, |left, right| { left > right });
            impl_int_relops!(le_s, $type, $type, |left, right| { left <= right });
            impl_int_relops!(le_u, $type_u, $type_u, |left, right| { left <= right });
            impl_int_relops!(ge_s, $type, $type, |left, right| { left >= right });
            impl_int_relops!(ge_u, $type_u, $type_u, |left, right| { left >= right });
